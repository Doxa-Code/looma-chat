...
import { z } from 'zod';
import { b as azureEmbeddings, p as pgVector, s as saveMessageOnThread } from '../index2.mjs';
...
const promotionProductsTool = createTool({
  ...
  async execute({ context, runtimeContext, threadId, resourceId }) {
    try {
      ...
      let result = "id,descri\xE7\xE3o,c\xF3digo,marca,pre\xE7o,estoque,pre\xE7o promocional,inicio da promo\xE7\xE3o,fim da promo\xE7\xE3o\n";
      result += products.filter((p) => p.stock > 0 && p.promotionEnd !== null).sort((a, b) => a.price > b.price ? 1 : -1).map((p) => [
        p.id,
        p.description,
        p.code,
        p.manufactory,
