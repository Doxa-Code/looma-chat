...
export class InitialPromptLeaf implements PromptLeaf {
  mount({ runtimeContext }: PromptLeafProps): string {
    ...
    return `
      ...
      - Caso não esteja, se desculpe educadamente e finalize o atendimento usando a ferramenta de fechar atendimento.
      - Caso esteja, adicione o endereço no pedido e prossiga com o atendimento.
      - Verifique se nas informações abaixo tem algo relevante para informar agora no inicio do atendimento: 
        ${settings.knowledgeBase ?? "Nenhum informação"}
      ...
      - Sempre feche o atendimento caso não atenda na localização do cliente, usando a ferramenta de fechar um atendimento.
    `;
  }
